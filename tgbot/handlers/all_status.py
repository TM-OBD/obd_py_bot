...
async def all_status1(message: Message):
    await message.answer("<code>Виберете машину:</code>", reply_markup=Inner_board.status_inlines()[0])


async def all_status2(call: CallbackQuery):
    await call.bot.edit_message_text("<b>Поточний статус машини:</b>\n\n"
                                     "<i>Заряд акумулятора:</i> невідомо❓\n<i>Рівень масла:</i> невідомо❓\n<i>Місцезнаходження:</i> невідомо❓\n<i>Поломоки:</i> немає✅",
